...
Update is built on [base][], a developer framework and API for rapidly building high quality node.js applications, using plugins like building blocks. This means that, in addition to plugins created just for Update, plugins from any [other Base application](#in-the-wild) can be used to add functionality to your updaters as well. 
